# SEO Инструкция для DRAWBRIDGE

## Созданные файлы

### ✅ robots.txt
- Разрешает индексацию всего сайта
- Блокирует служебные директории
- Ограничивает частоту сканирования для агрессивных ботов
- Указывает путь к sitemap

### ✅ sitemap.xml
- Карта сайта для поисковых систем
- Включает главную страницу и английскую версию
- Указаны приоритеты и частота обновлений
- **Важно**: обновляйте дату `lastmod` при изменении контента

### ✅ home-ita.html
- Английская версия страницы (нужна, чтобы `hreflang` и `sitemap.xml` не ссылались на 404)
- Должна быть доступна по URL: `https://drawbridge.kz/home-ita.html`

### ✅ Страницы услуг (лендинги под запросы)
- `website-development.html` — разработка сайтов
- `mobile-app-development.html` — разработка мобильных приложений
- `1c-integration.html` — интеграция с 1С
- `erp-integration.html` — интеграция ERP (обмен данными)
- `ai-llm.html` — AI/LLM решения
- `iot.html` — IoT и встраиваемые системы
- `devops.html` — DevOps и инфраструктура
- `cloud-migration.html` — миграция в облако
- `api-development.html` — разработка API
- `backend-development.html` — backend разработка
- `frontend-development.html` — frontend разработка
- `ui-ux-design.html` — UI/UX дизайн
- `design-system.html` — дизайн‑системы
- `landing-page.html` — лендинги
- `corporate-site.html` — корпоративные сайты
- `ecommerce-development.html` — интернет‑магазины
- `payment-integration.html` — интеграция оплат
- `crm-implementation.html` — внедрение CRM
- `qa-testing.html` — QA тестирование
- `data-analytics.html` — аналитика/BI
- `performance-optimization.html` — оптимизация скорости/Core Web Vitals
- `seo-optimization.html` — техническое SEO
- `chatbot-development.html` — чат‑боты
- `telegram-bot-development.html` — Telegram‑боты
- `website-support.html` — поддержка сайта
- `mobile-app-support.html` — поддержка мобильного приложения

**Важно:** после добавления/изменения таких страниц обновляйте [sitemap.xml](sitemap.xml) и держите внутренние ссылки на них с главной страницы.

### ✅ .htaccess
- Редирект HTTP → HTTPS
- Редирект www → без www
- GZIP сжатие для ускорения загрузки
- Кеширование браузера
- Заголовки безопасности
- Удаление .html из URL

## Следующие шаги

## Если Google пишет «не индексируется»

### «Вариант страницы с тегом canonical»
Это не ошибка как таковая. Обычно означает, что Google нашёл дубль страницы по другому URL (например, с/без `.html`, со слэшем на конце), но увидел `rel=canonical` и выбрал канонический URL.

Что делать:
- Если дубли не нужны — лучше настроить **301 редирект** со всех «альтернативных» URL на канонический.
- В этом репозитории канонические URL у страниц — с расширением `.html` (например `https://drawbridge.kz/frontend-development.html`).
- Файл [.htaccess](.htaccess) настроен так, чтобы URL **без `.html`** и URL **со слэшем на конце** редиректились на канонический `.html`. Это обычно снижает количество исключений «Вариант страницы с тегом canonical» в Search Console.

### «Не найдено (404)»
Это означает, что Google пытается открыть URL, но сервер отвечает 404.

Частые причины:
- Старые URL из внешних ссылок/закладок/старых sitemap.
- URL без `.html` или со слэшем на конце (например `/frontend-development` или `/frontend-development/`).

Что делать:
- Откройте в Search Console отчёт по 404 и посмотрите конкретные примеры URL.
- Если это «варианты» существующих страниц (без `.html` / со слэшем) — их закрывает редирект в [.htaccess](.htaccess).
- Если это реально удалённые/переименованные страницы — либо верните файл, либо сделайте 301 редирект на ближайшую релевантную страницу (на уровне сервера/хостинга).
- После исправлений: в Search Console используйте «Проверка URL» → «Запросить индексирование» (для важных страниц) и дождитесь переобхода.

### 1. Google Search Console
1. Зарегистрируйтесь на https://search.google.com/search-console
2. Добавьте сайт drawbridge.kz
3. Подтвердите владение сайтом
4. Отправьте sitemap.xml: `https://drawbridge.kz/sitemap.xml`

### Важно про hreflang
- Если вы меняете URL английской версии, обязательно синхронизируйте:
	- `link rel="alternate" hreflang=...` в [index.html](index.html)
	- записи и `xhtml:link` в [sitemap.xml](sitemap.xml)
	- сам файл английской страницы

### 2. Google Business Profile
1. Создайте профиль на https://business.google.com
2. Укажите адрес в Алматы
3. Добавьте фото офиса/команды
4. Заполните все поля (часы работы, услуги)
5. Получайте и отвечайте на отзывы

### 3. Yandex Вебмастер
1. Зарегистрируйтесь на https://webmaster.yandex.ru
2. Добавьте сайт
3. Отправьте sitemap.xml

### 4. Контент-стратегия
**Создайте блог** со статьями:
- "Как выбрать IT-подрядчика в Казахстане"
- "Интеграция с 1С: пошаговое руководство"
- "AI решения для малого бизнеса"
- Кейсы выполненных проектов
- Обзоры технологий

**Частота**: минимум 2-4 статьи в месяц

### 5. Технические улучшения
- [ ] Проверьте скорость на https://pagespeed.web.dev
- [ ] Оптимизируйте изображения (используйте WebP)
- [ ] Добавьте lazy loading для изображений
- [ ] Убедитесь, что SSL сертификат активен

### 5.1 Schema.org (структурированные данные)
- На главной добавлена разметка `FAQPage` по секции «частые вопросы» — это помогает Google понимать вопросы/ответы.
- Убран `aggregateRating` из JSON-LD, т.к. без явных отзывов/источника рейтинга на странице такой блок часто считается «самостоятельным рейтингом» и обычно игнорируется.

### 6. Обратные ссылки (Backlinks)
Разместите информацию о компании:
- **Каталоги**: 2GIS, Yandex.Maps, Google Maps
- **IT-платформы**: VC.ru, Habr, dev.to
- **Социальные сети**: LinkedIn, Facebook
- **Бизнес-каталоги**: казахстанские бизнес-справочники

### 7. Локальное SEO
- Используйте ключевые слова: "IT компания Алматы", "разработка ПО Казахстан"
- Указывайте город в контенте
- Получайте отзывы от казахстанских клиентов

### 8. Аналитика
Отслеживайте:
- Позиции по ключевым запросам
- Трафик из поисковых систем
- Конверсии (заявки с сайта)
- Поведение пользователей

### 9. Ключевые запросы для продвижения
- разработка по казахстан
- веб разработка алматы
- it компания алматы
- мобильные приложения казахстан
- интеграция 1с
- ai решения для бизнеса
- iot разработка
- devops услуги

## Ожидаемые результаты

- **1-2 месяца**: индексация сайта, первые позиции по низкочастотным запросам
- **3-4 месяца**: рост трафика, позиции в топ-30
- **6+ месяцев**: топ-10 по целевым запросам при регулярной работе

## Важно
SEO — это марафон, не спринт. Главное — регулярность и качество контента.
